...
def nor(x):
    ...
    norm_tensor = torch.nn.functional.normalize(x, p=2, dim=1)

    # 将归一化后的张量限制在0-1之间
    norm_tensor = (norm_tensor + 1) / 2
    return norm_tensor
...
#
#         y = torch.bmm(pos, x)
#         y = torch.bmm(y,torch.transpose(pos, 1, 2))
#         x = y / torch.einsum('bii->b',y).unsqueeze(-1).unsqueeze(-1)
#
#         return x
...
class QLModel(nn.Module):
    def __init__(self, args: object, plm: object, k1:object,k2:object) -> object:
        ...
        #self.pos =  nn.ModuleList([UT(32) for i in range(15)])
        ...
        # self.poolr = nn.AdaptiveAvgPool2d([1,1])
        # self.poolc = nn.AdaptiveAvgPool2d([1,1])
        # self.cnmip2 = nn.Conv2d(64, 1, 1, 1)
        # self.cnspv2 = nn.Conv2d(64, 1, 1, 1)
